 ...
 **/
 ...
 {
   LOG_INFO("BehaviorStackBlocks.BehaviorStackBlocks", "Running config");
 ...
 {
   LOG_INFO("BehaviorStackBlocks.InitBehavior", "Running init");
   ...
   DEV_ASSERT(_iConfig.pickupBehavior != nullptr,
              "BehaviorStackBlocks.InitBehavior.NullPickupBehavior");
 }
 ...
 { 
   LOG_INFO("BehaviorStackBlocks.GetAllDelegates", "Running get delegates");
...
//   return (nullptr != GetBEI().GetCubeInteractionTracker().GetTargetStatus().observableObject);
   ...
   LOG_INFO("BehaviorStackBlocks.WantsToBeActivatedBehavior", "Running get wants to be active");

   ObjectID bottomBlock;
   ObjectID topBlock;
   CalculateTargetBlocks(bottomBlock, topBlock);
   return true; //bottomBlock.IsSet() && topBlock.IsSet();
 }
 ...
 {
   LOG_INFO("BehaviorStackBlocks.OnBehaviorActivated", "Running behavior active");

   _dVars = DynamicVariables();
   CalculateTargetBlocks(_dVars.targetBlockBottom, _dVars.targetBlockTop);
 
   const auto& robotInfo = GetBEI().GetRobotInfo();
   if(robotInfo.GetCarryingComponent().GetCarryingObjectID() == _dVars.targetBlockTop){
     LOG_INFO("BehaviorStackBlocks.OnBehaviorActivated",
 ...
 {
   if(!IsActivated()){
      LOG_INFO("BehaviorStackBlocks.BehaviorUpdate",
               "Behavior isn't active! Not doing anything.");
     return;
   }
 
   LOG_INFO("BehaviorStackBlocks.BehaviorUpdate",
            "Behavior is active. Will do stuff.");
   ...
   if(!(topValid && bottomValid)){
     ...
     if(!bottomValid){
       ...
       if((bottomObj != nullptr)  &&
          (topObj != nullptr)){
                // TODO: Adding BlockWorldFilter() to fix build but not sure if correct
         auto objOnTop = GetBEI().GetBlockWorld().FindLocatedObjectOnTopOf(*bottomObj, STACKED_HEIGHT_TOL_MM, BlockWorldFilter());
         if(objOnTop == topObj){
           shouldStop = false;
         }
       }
 ...
 {
   LOG_INFO("BehaviorStackBlocks.TransitionToPickingUpBlock",
            "Will set state.");
 ...
 void BehaviorStackBlocks::TransitionToStackingBlock()
 {
   ...
   const bool holdingTopBlock = robotInfo.GetCarryingComponent().IsCarryingObject() &&
                                robotInfo.GetCarryingComponent().GetCarryingObjectID() == _dVars.targetBlockTop;
   if( ! holdingTopBlock ) {
